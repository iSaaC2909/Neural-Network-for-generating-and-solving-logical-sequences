...
void train(NeuralNetwork& model, const Data& trainData, const Data& valData, int epochs, float learningRate) {
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (size_t i = 0; i < valData.inputs.size(); ++i) {
            ...
            std::vector<float> target(valData.targets.begin() + i, valData.targets.begin() + i + 1);

            auto prediction = model.forward(input);
            float valLoss = computeLoss(prediction, target);
            totalValLoss += valLoss;
        }

        std::cout << "Epoch " << epoch + 1
                  << ", Training Loss: " << totalTrainLoss / trainData.inputs.size()
                  << ", Validation Loss: " << totalValLoss / valData.inputs.size() << "\n";
    }
}

void evaluate(const NeuralNetwork& model, const Data& testData) {
    float correct = 0;

    for (size_t i = 0; i < testData.inputs.size(); ++i) {
        auto input = normalize(testData.inputs[i]);
        auto prediction = model.forward(input);

        int predictedClass = std::distance(prediction.begin(), std::max_element(prediction.begin(), prediction.end()));
        if (predictedClass == testData.targets[i]) {
            correct++;
        }
    }

    std::cout << "Accuracy: " << (correct / testData.inputs.size()) * 100 << "%\n";
}
...
int main() {
    ...
    size_t trainSize = dataset.inputs.size() * trainRatio;
    size_t valSize = dataset.inputs.size() * valRatio;

    Data trainData = {
        std::vector<std::vector<int>>(dataset.inputs.begin(), dataset.inputs.begin() + trainSize),
        std::vector<int>(dataset.targets.begin(), dataset.targets.begin() + trainSize)
    };

    Data valData = {
        std::vector<std::vector<int>>(dataset.inputs.begin() + trainSize, dataset.inputs.begin() + trainSize + valSize),
        std::vector<int>(dataset.targets.begin() + trainSize, dataset.targets.begin() + trainSize + valSize)
    };

    Data testData = {
        std::vector<std::vector<int>>(dataset.inputs.begin() + trainSize + valSize, dataset.inputs.end()),
        std::vector<int>(dataset.targets.begin() + trainSize + valSize, dataset.targets.end())
    };
    ...
    train(model, trainData, valData, epochs, learningRate);
